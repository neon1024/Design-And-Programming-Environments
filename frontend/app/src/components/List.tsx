...
import {useCarsStore} from "../../../../backend/app/store.ts";
...
export default function List() {
    ...
    const cars = axios.get("http://localhost:3000/api/cars")
        .then((response) => {
    ...
    function removeSelectedItems() {
        selectedItems.forEach((selectedItem) => {
            remove(selectedItem);
            axios.delete(`http://localhost:3000/api/cars/:${selectedItem}`).then((response) => console.log(response)).catch((error) => console.log(error));
        });
